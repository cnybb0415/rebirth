...
export default async function ConcertFundingPage({
  ...
  const [FUND_PERCENT, t] = await Promise.all([getFundPercent(), getTranslations("concert")]);
  const notice = announcements.find((a) => a.id === "2");
  const dDay = getDDay();
  const filledCells = Math.round((FUND_PERCENT / 100) * TOTAL_CELLS);

  return (
    <BinderPage activeTab="funding" pixelFontFamily="'Mulmaru', 'PFStarDust', monospace" locale={locale}>
      <BinderHeading
        emoji="💰"
